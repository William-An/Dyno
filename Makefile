##############################################################################
# Makefile for Dyno http server framework
##############################################################################

# Variables
CC = gcc
# Support adding flag when call `make`
override CFLAGS += -std=c99 -Wall -g3 -fPIC	
...
$(BUILD)/%.out: $(BUILD)/%.o $(OBJS)| $(BUILD)
	...
	@$(CC) -o $@ -I $(INC) $(CFLAGS) $(LINK) $< $(filter-out $(EXCLUDE), $(OBJS))
	@echo "Done"
	@echo "----------------------------------------------------------------"
